    ...
    for (int tix = 0; tix < gnx; tix++) {
        for (int tiy = 0; tiy < gny; tiy++) {
            ...
            for (int j = 0; j < tileN; j++) {
                ...
                rec_buf[14] = ((uint8_t)rp.return_num & 0x0F) |
                              (((uint8_t)rp.number_of_returns & 0x0F) << 4);
                rec_buf[16] = (uint8_t)rp.class_id;
                wLE<int16_t>(rec_buf, 18, (int16_t)rp.scan_angle);
                wLE<uint16_t>(rec_buf, 30, (uint16_t)(rp.r));
                wLE<uint16_t>(rec_buf, 32, (uint16_t)(rp.g));
                wLE<uint16_t>(rec_buf, 34, (uint16_t)(rp.b));
